-- COMPLETE NEWSLETTERFY PLATFORM DATABASE SETUP (FIXED v2)
-- This script creates ALL tables needed for the entire platform
-- Run this script in Supabase SQL Editor to set up everything
-- FIXED: Changed 'current_date' parameter to 'base_date' and fixed foreign key ordering
...
-- Donation goals/campaigns table (CREATE FIRST)
...
-- Recurring donations table (CREATE FIRST)
CREATE TABLE IF NOT EXISTS recurring_donations (
  ...
  goal_id UUID REFERENCES donation_goals(id) ON DELETE SET NULL,
  amount DECIMAL(10,2) NOT NULL,
  currency TEXT DEFAULT 'usd',
...
-- Donations table with CORRECT schema (NOW with proper references)
...
BEGIN
  ...
  RAISE NOTICE '';
  RAISE NOTICE '🔧 FOREIGN KEY ORDERING FIXED!';
  RAISE NOTICE '✅ NO MORE CONSTRAINT ERRORS!';
END $$; 